 ...
 *
 * Disassembly of SSDT-6.aml, Fri Aug 31 22:47:55 2018
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00005F61 (24417)
 *     Revision         0x02
 *     Checksum         0xE6
 *     OEM ID           "SaSsdt"
 *     OEM Table ID     "SaSsdt "
...
DefinitionBlock ("", "SSDT", 2, "SaSsdt", "SaSsdt ", 0x00003000)
{
    ...
    External (_SB_.PCI0.PEG0.PEGP.NHDA, FieldUnitObj)    // (from opcode)
    ...
    OperationRegion (SANV, SystemMemory, 0x9A69DE18, 0x018F)
    ...
    Scope (\_SB.PCI0)
    {
        ...
        Method (PGON, 1, Serialized)
        {
            ...
            If (LEqual (Arg0, Zero))
            {
                ...
                Store (\_SB.PCI0.PEG0.PEGP.LTRE, LREN)
                Store (One, CEDR)
                Store (H0VI, W0VI)
                Store (H0DI, W0DI)
        ...
        Method (PGOF, 1, Serialized)
        {
            ...
            Store (LREN, \_SB.PCI0.PEG0.PEGP.LTRE)
            If (LEqual (Arg0, Zero))
            {
                Store (One, P0LD)
                Store (Zero, TCNT)
            ...
            ElseIf (LEqual (Arg1, One))
            {
                If (LEqual (Arg0, Zero))
                {
                    If (CondRefOf (\_SB.PCI0.PEG0.PEGP.SGPO))
                    {
                        ...
                        While (LEqual (\_SB.PCI0.PEG0.PEGP.SGPI (PWOK), Zero))
                        {
                            Sleep (One)
                        }
